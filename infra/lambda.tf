...
resource "aws_lambda_function" "lambda_create_purchase" {
  function_name = "celer-purchase-create"
  filename         = data.archive_file.file_lambda_createPurchase.output_path
  source_code_hash = data.archive_file.file_lambda_createPurchase.output_base64sha256
  role    = aws_iam_role.lambda-role.arn
  handler = "create_purchase.lambda_handler"
  runtime = var.python_version
  layers = [aws_lambda_layer_version.service_lambda_layer.arn]
  tags = {"App":"celer"}
}
...
resource "aws_lambda_function" "lambda_list_purchase" {
  function_name = "celer-purchase-list"
  filename         = data.archive_file.file_lambda_listPurchase.output_path
  source_code_hash = data.archive_file.file_lambda_listPurchase.output_base64sha256
  role    = aws_iam_role.lambda-role.arn
  handler = "list_purchase.lambda_handler"
  runtime = var.python_version
  layers = [aws_lambda_layer_version.service_lambda_layer.arn, aws_lambda_layer_version.util_lambda_layer.arn]
  tags = {"App":"celer"}
}

resource "aws_lambda_function" "lambda_update_purchase" {
  function_name = "celer-purchase-update"
  filename         = data.archive_file.file_lambda_updatePurchase.output_path
  source_code_hash = data.archive_file.file_lambda_updatePurchase.output_base64sha256
  role    = aws_iam_role.lambda-role.arn
  handler = "update_purchase.lambda_handler"
  runtime = var.python_version
  layers = [aws_lambda_layer_version.service_lambda_layer.arn]
  tags = {"App":"celer"}
}
